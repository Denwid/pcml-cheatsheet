...
A matrix $\bf{M}$ is positive semidefinite $\iff \forall$ nonzero vector $\bf{a}$, we have $\bf{a}^T \bf{M} \bf{a} > 0$

Jensen's inequality applied to $\log$: $\log ( \frac{\sum_{i=1}^n x_i}{n} ) \geq \frac{\sum_{i=1}^n \log(x_i)}{n}$

Matrix inversion lemma: $(\bf{PQ} + \lambda \bf{I}_N)^{-1}) \bf{P} = \bf{P}(\bf{QP} + \bf{I}_M)^{-1}$
...
\todo[inline]{explaining how to find MLE/MAP parameters with EM}
...
\item $\bf{K}$ should be positive semi-definite: $\forall$ nonzero vector $\bf{a}, \bf{a}^T \bf{K} \bf{a} > 0$.
\end{itemize}
...
\todo[inline]{Gaussian Mixture Models and maybe kMeans?}
...
\todo[inline]{Time series (did we cover that? :-) )}
