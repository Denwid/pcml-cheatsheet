...
\section{Kernel}
Basically, Kernels are a mean to measure distance of two vectors, e.g. we could write $\kappa(\mathbf{x}, \mathbf{y}) = \mathbf{x}^T \mathbf{y}$ which would be the Kernel function for the standard inner product.
